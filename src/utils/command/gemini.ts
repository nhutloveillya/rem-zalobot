  ...
  bot.command("ai", async (message, context) => {
    ...
    if (message.admin) {
      if (ctx && ctx.length > 0) {
        ...
        } else if (ctx [0] === "reset") {//reset kí ức
            bot.sendChatAction(message.chat.id, "typing");
            try {
        ...
        }else if (ctx [0] === "status") {//hien trang thai cache
            bot.sendChatAction(message.chat.id, "typing");
            try {
    ...
    message.replyText(`Mày không phải là Admin, mày định làm gì Rem của tao?! Cút!!`)
  }
});
};
